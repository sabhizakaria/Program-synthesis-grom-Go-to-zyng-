...
__attribute__((sdx_kernel("axiStreamExample", 0))) void axiStreamExample (AXIS_STREAM producer[5], AXIS_STREAM consumer[5]);
# 2 "com_chan_axis.cpp" 2
# 20 "com_chan_axis.cpp"
__attribute__((sdx_kernel("axiStreamExample", 0))) void axiStreamExample(AXIS_STREAM producer[5], AXIS_STREAM consumer[5]){_ssdm_SpecArrayDimSize(producer, 5);_ssdm_SpecArrayDimSize(consumer, 5);
#pragma HLS TOP name=axiStreamExample
# 20 "com_chan_axis.cpp"
...
#pragma HLS INTERFACE axis port=producer
#pragma HLS INTERFACE axis port=consumer

 VITIS_LOOP_32_1: for (i = 0 ; i < 5 ; i++){
  consumer[i].data = producer[i].data.to_int()*i;
  consumer[i].keep = producer[i].keep;
  consumer[i].strb = producer[i].strb;
  consumer[i].user = producer[i].user;
  consumer[i].last = producer[i].last;
  consumer[i].id = producer[i].id;
  consumer[i].dest = producer[i].dest;
 }
